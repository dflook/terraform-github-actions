...
if [[ "$TERRAFORM_BACKEND_TYPE" == "remote" || "$TERRAFORM_BACKEND_TYPE" == "cloud" ]]; then
    ...
    debug_log $TOOL_COMMAND_NAME workspace delete -no-color -lock-timeout=300s "$INPUT_WORKSPACE"
    # shellcheck disable=SC2086
    (cd "$INPUT_PATH" && $TOOL_COMMAND_NAME workspace delete -no-color -lock-timeout=300s "$INPUT_WORKSPACE")
fi
