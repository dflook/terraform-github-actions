...
function set-plan-args() {
    ...
    if [[ -n "${INPUT_VAR:-}" ]]; then
        error_log "The var input has been removed. Use the variables input instead. See https://github.com/dflook/terraform-github-actions/blob/main/CHANGELOG.md"
        exit 1
    fi
}
...
function create-auto-tfvars() {
    ...
    if [[ -n "${INPUT_VAR_FILE:-}" ]]; then
        for file_path in $(echo "$INPUT_VAR_FILE" | tr ',' '\n'); do
            if [[ ! -f "$file_path" ]]; then
                error_log "Path does not exist: \"$file_path\""
                exit 1
            fi
