...
function plan() {
    ...
    set +e
    (cd $INPUT_PATH && ./deploy plan -input=false -no-color -detailed-exitcode -lock-timeout=300s $PLAN_OUT_ARG $PLAN_ARGS) \
...
function apply() {
    ...
    set +e
    (cd $INPUT_PATH && ./deploy apply -input=false -no-color -auto-approve -lock-timeout=300s $PLAN_OUT) | $TFMASK
