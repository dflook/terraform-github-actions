...
This actions runs an OpenTofu apply operation in refresh-only mode.
This will synchronise the OpenTofu state with the actual resources, but will not make any changes to the resources.

## Inputs

* `path`

  Path to the OpenTofu root module to refresh state for
...
* `workspace`
  ...
  OpenTofu workspace to run the refresh state in
